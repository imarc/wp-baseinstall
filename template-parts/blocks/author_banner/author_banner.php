...
$author_banner_profile = get_field('author_banner_profile');
$author_banner_eyebrow = get_field('author_banner_eyebrow');
$author_background_image = get_field('author_banner_background_image');
...
if( !empty( $block['data']['_is_preview'] ) ) { ?>
    <figure style="padding:0; margin:0;">
        <img src="<?php echo get_stylesheet_directory_uri(); ?>/template-parts/blocks/author_banner/author_banner-previsew.jpg" alt="Preview" style="display:block; width:100%; height:auto; margin:0; border: 1px solid #ddd;">
    </figure>

<?php } else { ?>
        
    <div id="<?php echo esc_attr($id); ?>" class="authorPage__wrapper <?php if(!empty($background_color)){echo ' -customBgColor';} ?> customBlock">


        <?php // author info 
        $authorHeadshot = get_avatar(get_the_author_meta('user_email', $author_banner_profile ), '200'); // Display author image, size of 200 
        $authorBio = get_the_author_meta('description', $author_banner_profile ); // Displays the author description added in Biographical Info
        $authorName = get_the_author_meta('display_name', $author_banner_profile ); // Displays the author name of the posts
        $authorWebsite = get_the_author_meta( 'user_url', $author_banner_profile );
        $authorFacebook = get_the_author_meta( 'facebook', $author_banner_profile );
        $authorInstagram = get_the_author_meta( 'instagram', $author_banner_profile );
        $authorLinkedIn = get_the_author_meta( 'linkedin', $author_banner_profile );
        $authorMySpace = get_the_author_meta( 'myspace', $author_banner_profile );
        $authorPinterest = get_the_author_meta( 'pinterest', $author_banner_profile );
        $authorSoundCloud = get_the_author_meta( 'soundcloud', $author_banner_profile );
        $authorTumblr = get_the_author_meta( 'tumblr', $author_banner_profile );
        $authorTwitter = get_the_author_meta( 'twitter', $author_banner_profile );
        $authorYouTube = get_the_author_meta( 'youtube', $author_banner_profile );
        $authorWikipedia = get_the_author_meta( 'wikipedia', $author_banner_profile ); 
        ?>
            ...
            <div class="authorPage">
                ...
                if ($authorName) { ?>
                    ...
                    <div class="authorPage_info">
                        ...
                        <?php if (!empty($author_banner_eyebrow)) { ?>
                            <div class="authorPage_info-eyebrow <?php if(!empty($background_color)){echo ' -customColor';} ?>">
                                <?php echo $author_banner_eyebrow; ?>
                            </div>
                        <?php } ?>

                        <?php if (!empty($authorName)) { ?>
                            <div class="authorPage_info-name <?php if(!empty($background_color)){echo ' -customColor';} ?>">
                                <a href="<?php echo esc_url( get_author_posts_url( $author_banner_profile ) ); ?>">
                                <?php echo $authorName; ?>
                                </a>
                            </div>
                        <?php } ?>
                        ...
                        <?php if (!empty($authorBio)) { ?>
                            <div class="authorPage_info-bio <?php if(!empty($background_color)){echo ' -customColor';} ?>">
                                <?php echo $authorBio; ?>
                            </div>
                        <?php } ?>
        ...
        <?php // color and padding options from WP block editor - due to the variable width and alignment for this block in the comp, styles are applied to the 'ctaBanner__block' element, and the usual padding has been replaced with margins that only apply to the narrow-spaced option 
            if (!empty($background_color || $text_color || $top_bottom_padding || $author_background_image)) { ?>
            <style type="text/css">
                <?php echo '#' . $id . ' .' . $className; ?> { <?php

                    // background image 
                    if (!empty($author_background_image)) { 
                        echo 'background-image: url(' . $author_background_image . ');'; } 
